...
router.get('/:slug/stats/me', verifyToken, (req, res) => {
    ...
    // Votes received on my songs (upvotes - downvotes)
    ...
    WHERE s.room_id = ? AND s.added_by = ?
  `).get(room.id, userId);

    // My total vote score across all my songs
    const totalScore = db.prepare(
        'SELECT COALESCE(SUM(vote_score), 0) as score FROM songs WHERE room_id = ? AND added_by = ?'
    ).get(room.id, userId).score;

    // Messages sent
    const messageCount = db.prepare(
        'SELECT COUNT(*) as count FROM chat_messages WHERE room_id = ? AND user_id = ?'
    ).get(room.id, userId).count;
    ...
    res.json({
        stats: {
            songsInQueue,
            songsTotal,
            upvotesReceived: votesReceived.upvotes,
            downvotesReceived: votesReceived.downvotes,
            totalVoteScore: totalScore,
            messageCount,
        },
    });
});
...
router.get('/:slug/leaderboard', verifyToken, (req, res) => {
    ...
    // Aggregate: top members by total vote score of songs they added
    ...
    SELECT
      ...
      COALESCE(SUM(
        (SELECT COALESCE(SUM(CASE WHEN v.type = 'up' THEN 1 WHEN v.type = 'down' THEN -1 ELSE 0 END), 0)
         FROM votes v JOIN songs s2 ON v.song_id = s2.id WHERE s2.added_by = u.id AND s2.room_id = ?)
      ), 0) as current_vote_score,
    ...
    GROUP BY u.id
    ORDER BY total_songs DESC, current_vote_score DESC
    LIMIT 10
  `).all(room.id, room.id, room.id, weekAgo);
    ...
    if (leaderboard.length === 0) {
      ...
      SELECT
        ...
        COALESCE(SUM(s.vote_score), 0) as current_vote_score,
        0 as songs_played
      FROM songs s
      JOIN users u ON s.added_by = u.id
      WHERE s.room_id = ?
      GROUP BY u.id
      ORDER BY current_vote_score DESC, total_songs DESC
      LIMIT 10
    `).all(room.id);
    }
