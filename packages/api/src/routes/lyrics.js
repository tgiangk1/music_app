...
router.get('/', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const searchRes = await fetch(searchUrl, { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; JukeboxBot/1.0)', 'Accept': 'application/json' } });
        ...
        const pageRes = await fetch(pageUrl, { headers: { 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36', 'Accept': 'text/html,application/xhtml+xml' } });
        ...
        const songInfo = { title: hits[0]?.result?.title || query, artist: hits[0]?.result?.primary_artist?.name || '', thumbnail: hits[0]?.result?.song_art_image_thumbnail_url || '' };
